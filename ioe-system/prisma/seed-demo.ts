...
async function main() {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        console.log(`Created SO: ${so.erpReference}`);
    }
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        const po = await prisma.order.create({
            data: {
                ...
                customerName: supplier.name, // In this model, Supplier acts as "Customer" for the order record
                originId: "Supplier Dock",
                destination: { street: "Our Warehouse", city: "Atlanta", state: "GA", zip: "30303", country: "USA" }, // Inbound to us
                status: "PLANNED",
                priority: "NORMAL",
        ...
        console.log(`Created PO: ${po.erpReference}`);
    }
